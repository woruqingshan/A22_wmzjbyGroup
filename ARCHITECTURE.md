...
└───────────────────────────┬─────────────────────────────────┘
                            ...
                            │ 例: 本地 127.0.0.1:19000 -> 远端 127.0.0.1:9000
                            ▼
┌─────────────────────────────────────────────────────────────┐
│ 实验室 / 远程服务器 (推理与编排层)                          │
...
当远程 HTTP 服务仅监听 **远端本机**（例如 `127.0.0.1:9000`）时，在 **WSL 本地** 建立 SSH 本地端口转发：

```bash
ssh -N -L 127.0.0.1:19000:127.0.0.1:9000 <user>@<lab-host>
```

含义：

- 本地监听 `127.0.0.1:19000`
- 流量经 SSH 加密通道到远端 `127.0.0.1:9000`
