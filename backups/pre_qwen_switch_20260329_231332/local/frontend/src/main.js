...
async function handleSend({ text, audio, video }) {
  ...
  try {
    ...
    chatPanel.addMessage({
      ...
      meta: `${response.emotion_style} 鐠?${response.avatar_action.facial_expression} / ${response.avatar_action.head_motion}`,
    });
    if (hasAudio && response.transcript_text) {
      chatPanel.addSystemMessage(`ASR transcript: ${response.transcript_text}`);
    }
